...
<tbody>
	<c:forEach items="${tasklist }" var="leave">
		...
		<tr id="${leave.id }" align="center">
			...
			<td>
				<a class="trace" href='${ctx }/workflow/view/${task.executionId}/page/${task.processInstanceId}.do' title="点击查看流程图">${task.name }</a>
			</td>
			...
			<td>
				<c:if test="${empty task.assignee }">
					<a class="claim" href="${ctx }/leave/task/${task.id}/claim.do" target="main">签收</a>
				</c:if>
				<c:if test="${not empty task.assignee }">
					<a href="${ctx }/leave/detail/leave/${task.id}.do">办理</a>
				</c:if>
			</td>
		</tr>
	</c:forEach>
