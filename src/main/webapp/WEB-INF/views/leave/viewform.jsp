...
目录lang 存放语言文件 
目录skin 存放皮肤的相关文件
当WdatePicker.js里的属性:$wdate=true时,在input里加上
class="Wdate"就会在选择框右边出现日期图标,如果您不喜欢这个样式,
可以把class="Wdate"去掉,另外也可以通过修改skin目录下的WdatePicker.css文件来修改样式-->
<script language="javascript" type="text/javascript" src="${ctx }/My97DatePicker/WdatePicker.js"></script>
<script type="text/javascript">
$(function() {
	//根据activitiId显示相应的信息
	$('#${activityId }').css("display","inline");//此元素会被显示为内联元素，元素前后没有换行符。
});
/**
 * 完成任务
 * @param {Object} taskId
...
function complete(taskId, variables) {
	...
	var url="${ctx}/leave/task/"+taskId+"/complete.do";
	// 发送任务完成请求
    $.post(url,{
        keys: keys,
        values: values,
...
function deptLeaderAudit(){
	var deptLeaderPass=$('#deptLeaderAudit :radio[name=deptLeaderPass]:checked').val(); //经理意见
	var deptauditreason=$('#deptLeaderAudit #deptauditreason').val();//经理给出的理由
	complete('${taskId}',[
		{
			key: 'deptLeaderPass',
